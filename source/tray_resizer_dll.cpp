// tray_resizer_dll.cpp — Simplified: timer + window property XamlRoot enumeration
// No pattern scanning, no DbgHelp, no RTTI, no MinHook, no internal functions.
// Periodically enumerates DesktopWindowContentBridge window properties
// looking for the DesktopWindowXamlSource -> get XamlRoot -> apply styles.
...
// Simple log
static void LOG(const wchar_t* s) { OutputDebugStringW(s); }

// ╔══════════════════════════════════════════════════════════╗
// ║  XAML helper functions                                  ║
// ╚══════════════════════════════════════════════════════════╝
...
// Style functions from original mod
...
// ╔══════════════════════════════════════════════════════════╗
// ║  Enumerate bridge window properties for XamlRoot        ║
// ╚══════════════════════════════════════════════════════════╝

struct PropCtx { HWND hwnd; XamlRoot result; };

// Static callback for EnumPropsEx - tries each property as FrameworkElement
static BOOL CALLBACK PropEnumCB(HWND, LPCWSTR, HANDLE hData, ULONG_PTR lParam) {
    auto& ctx = *(PropCtx*)lParam;
    if (!hData || hData == INVALID_HANDLE_VALUE) return TRUE;
    IUnknown* unk = (IUnknown*)hData;
    __try {
        unk->AddRef(); 
        if (unk->Release() == 0 || unk->Release() > 100000) return TRUE; // invalid
        FrameworkElement fe = nullptr;
        if (SUCCEEDED(unk->QueryInterface(winrt::guid_of<FrameworkElement>(), winrt::put_abi(fe))) && fe) {
            ctx.result = fe.XamlRoot();
            if (ctx.result) return FALSE; // found!
        }
...
static XamlRoot FindXamlRoot() {
    ...
    // Check all child windows for XAML bridge windows
    PropCtx ctx = {nullptr, nullptr};
    EnumChildWindows(hTray, [](HWND h, LPARAM l)->BOOL{
        ...
        // Enumerate this window's properties
        PropCtx pc = {h,nullptr};
        EnumPropsEx(h, PropEnumCB, (LPARAM)&pc);
        if(pc.result) { *(XamlRoot*)((PropCtx*)l)->result = pc.result; return FALSE; }
        return TRUE;
    }, (LPARAM)&ctx);
...
static VOID CALLBACK OT(HWND,UINT,UINT_PTR id,DWORD){
    if(g_u){KillTimer(nullptr,id);return;}
    auto xr=FindXamlRoot();
    if(xr) ApplyFromRoot(xr,ICON_W,ICON_R);
}
DWORD WINAPI Wkr(LPVOID){
    LOG(L"tray_resizer: loaded\r\n");
    CoInitializeEx(nullptr,COINIT_APARTMENTTHREADED);
    auto xr=FindXamlRoot();
    if(xr){ApplyFromRoot(xr,ICON_W,ICON_R);LOG(L"tray_resizer: applied\r\n");}
    else LOG(L"tray_resizer: no xamlroot\r\n");
